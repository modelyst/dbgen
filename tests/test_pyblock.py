...
def test_pyblock_to_dict():
    pb = PyBlock(env=Environment(imports=[]), function=nonary, inputs=[], outputs=[""])
    serial = pb.dict()
...
def test_pyblock_run_with_inputs():
    func = lambda x, y: x + y
    pb = PyBlock(function=func, inputs=[Constant(1), Constant(3)])
    pb_dict = pb.run({})
    assert "out" in pb_dict
...
def test_pyblock_run_with_args():
    func = lambda x, y: x + y
    pb = PyBlock(function=func, inputs=[Constant(1), Arg(key="other_pyblock", name="out")])
...
def test_two_pyblocks():
    ...
    pb_1 = PyBlock(function=func_1, inputs=[Constant(1)])
...
def test_pyblock_failure_at_runtime():
    ...
    with pytest.raises(ValidationError):
        PyBlock(function=nonary, inputs=[Constant(1)], outputs=["1", "2"])

    with pytest.raises(ValidationError):
        PyBlock(function=nonary, inputs=[Constant(1)], outputs=["1", "2"])

    PyBlock(function=ternary, inputs=[Constant(1), Constant(2)], outputs=["1", "2"])
