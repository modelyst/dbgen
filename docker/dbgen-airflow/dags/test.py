...
# Internal Imports
from dbgen import Generator
from dbgen.core.airflow_plugin import GenOperator  # type: ignore

# Written by mstatt

objs = dict(
    roots=("roots_id", ["root", "code", "cluster", "label"], []),
    job=("job_id", ["stordir"], []),
    element=("element_id", ["atomic_number"], []),
    cell=("cell_id", ["a1", "a2", "a3", "b1", "b2", "b3", "c1", "c2", "c3"], []),
    struct=("struct_id", ["raw"], []),
    atom=("atom_id", ["ind"], ["struct"]),
    struct_composition=("struct_composition_id", [], ["struct", "element"]),
    surface=("surface_id", [], ["struct"]),
    bulk=("bulk_id", [], ["struct"]),
    molecule=("molecule_id", [], ["struct"]),
    calc=("calc_id", ["dftcode", "xc", "pw", "psp"], []),
    calc_other=(
        "calc_other_id",
        [
            "kx",
    ...
    relax_job=("relax_job_id", [], ["job"]),
    vib_job=("vib_job_id", [], ["job"]),
    vib_modes=("vib_modes_id", ["ind"], ["vib_job"]),
    traj=("traj_id", ["step"], ["relax_job"]),
    traj_atom=("traj_atom_id", [], ["traj", "atom"]),
)  # type: D[str,T[str,L[str],L[str]]]
...
default_args.update()
with DAG(
    "simple_catalog_model", schedule_interval="@once", default_args=default_args
) as dag:
