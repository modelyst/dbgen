...
class Generator(Base):
    ...
    def transform_batch(self, batch: List[Tuple[UUID, Dict[str, Dict[str, Any]]]]):
        ...
        rows_to_load: Dict[str, Dict[str, Dict[str, Any]]] = {node.hash: {} for node in self.loads}
        processed_hashes = []
        inputs_skipped = 0
        for input_hash, row in batch:
            try:
                ...
                # self._logger.debug(f'encountered error during processing of row:\n{row}')
    ...
    def _transform(self, namespace: dict, rows_to_load: Dict[str, Dict[str, dict]]):
        skipped = False
        try:
